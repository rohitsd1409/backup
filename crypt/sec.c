...
char* get_key()
{
	...
	key = calloc(MAX_KEY+1, sizeof(char));

	printf("Enter Authentication Key (Min: %d and Max %d characters): ", MIN_KEY, MAX_KEY);
	while(c = getchar())
	{
		printf("i: %d\n", i);
		...
		//If user enter's string of length equievalent to max limit i.e. MAX_KEY in this case;
		//Then, it's a valid input only if the user enters '\n' or EOF after that;
		//Eg:	Let, MAX_KEY = 5
		//	In this case:
		//		Input: abcde\n  is valid 
		//		      \____/\_/
		...
		//Or USer has entered MAX_KEY + 1 'th char but it is not '\n' or EOF
		else if(i >= MAX_KEY)
		{
