...
void send_backup_data(int sock, char* abs_src, char* abs_dest)
{
	...
	while((dent = readdir(src_dir)) != NULL)
	{
		...
		if(!S_ISDIR(cur_file_stat->st_mode))
		{
			printf("Waiting for Contents Transfer Confirmation..\n");
			...
			if(transfer_file_contents == 'y')
			{
				...
				{
					error = 0;
					printf("ERROR occured while sending/reading file contents..\n");
		...
		//Preserve timestamps of the src. file (since, it's access time is modified during backup operation, resotre it to the original val)
		preserve_timestamps(abs_src_file, &bf_stat->atime, &bf_stat->mtime);
...
void* recv_backup_data(void* arg)
{
	...
	int fd				= 0;
	int bufsize			= 0;
	int tentative_backup_ctr	= 0;
	int i				= 0;
	struct stat stat_buf		= {0};
	char* cur_backup_dir 		= NULL;		//abs path of backup dir in which current backup operation will take place. Eg: /abc/def/1223..22
	char* relative_filepath		= NULL;		//relative path of file w.r.t any backup dir. EgL /abc/def/122..32/dir1/f1 
							//means relative path of f1 is dir1/f1
	char* prev_abs_path		= NULL;		//abs path where file would have been saved prev. if it were backed up then.
	...
	int n;
	int total = sizeof(temp_bop);
	while((n = recv(sock_fd, &temp_bop, sizeof(temp_bop), 0)))
	{
		printf("n: %d\n", n);
		if(n > 0)
		{
	...
	//which backup number is this?
	tentative_backup_ctr = (temp_bop.b_ctr + 1)%NBACKUPS;
	...
	while(1)
	{
		...
		int xbytes;
		if(sizeof(struct backup_file_stat) != (xbytes = recv(sock_fd, &bf_stat, sizeof(struct backup_file_stat), 0)))
		{
			perror("recv");
			if(errno == EINTR)
			{
			...
			printf("XBYTES: %d sizeof(backup_file_struct): %d\n", xbytes, sizeof(struct backup_file_stat));
		...
		else
		{
			...
			if(0 == tentative_backup_ctr)
			{
				transfer_file_contents = 'y';	//as of now, file must be backed up
				printf("Full backup will be performed..\n");
			}
			else
			{
				//////
				//check prev. backups to find out whether file needs to be backed up or not
				//////	
				i = tentative_backup_ctr - 1;
				//Find relative filepath
				//Eg:	cur_backup_dir = /abc/def/2122..12/
				//	abs_dest       = /abc/def/2122..12/dfg/fdf/d1/f1
				//  Therefore,	relative file path of a file "f1" inside all backup dir's is:
				//		/dfg/fdf/d1/f1
				cur_backup_dir    = abs_path(0, temp_bop.dest, backup_dirname, "/", NULL); 
				relative_filepath = str_diff(bf_stat.abs_dest, cur_backup_dir);
				printf("cur_backup_dir: 	%s\n", cur_backup_dir);
				printf("relative_filepath: 	%s\n", relative_filepath);

				while(i >= 0)
				{
					//obtain abs_path of file during i'th backup
					prev_abs_path = abs_path(0, temp_bop.dest, temp_bop.last_bt[i], "/", relative_filepath, NULL);
					printf("i: %d prev_abs_path: %s\n", i, prev_abs_path);

					//file stats can't be accessed, probaly file doesnot exists
					if(-1 == stat(prev_abs_path, &stat_buf))
					{
						--i;	//check prev. backup
						transfer_file_contents = 'y';	//as of now, file must be backed up
						free(prev_abs_path);
					...
					else
					{
						...
						if((stat_buf.st_mtime == bf_stat.mtime)  &&
						  (stat_buf.st_mode  == bf_stat.mode)   &&
						  (stat_buf.st_uid   == bf_stat.uid)    &&
						  (stat_buf.st_gid   == bf_stat.gid))
						{
							printf("File unchanged here: %s..Hence won't be backed..\n", prev_abs_path);
							transfer_file_contents = 'n';	
							free(prev_abs_path);
							break;
						}
						else
						{
							printf("File changed here: %s..Hence must be backed..\n", prev_abs_path);
							transfer_file_contents = 'y';
							free(prev_abs_path);
							break;
						}
					}
				}//end of while(test all prev. backups to check whether file should be backed up or not)	
			}//else (tentative backup is non-zero)

			//send confirmation whether file contents should be sent to remote daemon or not
			while(1 != send(sock_fd, &transfer_file_contents, sizeof(char), 0))
			{
				perror("send");
				if(errno == EINTR)
				{
			...
			if(transfer_file_contents == 'y')
			{
				...
				nbytes = bf_stat.size;
				printf("File : %s nbytes: %d\n", bf_stat.abs_dest, nbytes);
				bufsize = nbytes > BUFSIZE ? BUFSIZE : nbytes;	//very imp.(don't read content that belongs to someother file/struct)
				while((nbytes > 0) && (temp = recv(sock_fd, buf, bufsize, 0)))	//read file contents from the socket
				{
					...
					while(temp != write(fd, buf, temp))	//write file contents into the file 
					{
						perror("send");
						if(errno == EINTR)
						{
					...
					nbytes -= temp;
					printf("File : %s Read bytes: %d\n", bf_stat.abs_dest, temp);
					if(nbytes < bufsize)	//very imp.(don't read content that belongs to someother file/struct)
					{
						bufsize = nbytes;
					}
				}
				...
				printf("Successfully Received backup file contents(%s) by remote Daemon..\n", bf_stat.abs_dest);
				close_descriptor(fd, 0);
				//Ownership should be preserved first and timestamps at the last,
				//otherwise modification time of file will be set to the time
				//when file's ownership is preserved
				preserve_ownership(bf_stat.abs_dest, &bf_stat.uid, &bf_stat.gid);
				preserve_timestamps(bf_stat.abs_dest, &bf_stat.atime, &bf_stat.mtime);
				printf("Reading next file..\n");
			}//end of the transfer of file contents
...
*/			
		...
		printf("Port: %d\n", daemon_rh.sin_port);
		if(-1 == inet_pton(AF_INET, "192.168.2.2", &daemon_rh.sin_addr))
		{
			perror("inet_pton");
			close_descriptor(sock_fd_lh, 0);	
			close_descriptor(sock_fd_rh, 1);	
